...
function AttackMode.Event(event)
  -- BotLogger.debug("AttackMode.Event(event) ")
  -- Cannot continue if still attacking or in pz
  local player = g_game.getLocalPlayer()
  -- BotLogger.debug("AttackMode.Event(event), g_game.isAttacking() "..tostring(g_game.isAttacking()))
  ...
  if creature == nil then
    BotLogger.debug("AttackMode.Event(event), creature == nil")
    return Helper.safeDelay(300, 500)
  end
  BotLogger.debug("AttackMode.Event(event), creature ~= nil")
  local target = TargetsModule.getTarget(creature:getName())
  BotLogger.debug("AttackMode.Event(event), target==nil: "..tostring(target==nil))
  if target then
    for index, setting in ipairs(target:getSettings()) do
      -- BotLogger.debug("AttackMode.Event(event), index "..tostring(index).." setting "..table.ToString(setting))
